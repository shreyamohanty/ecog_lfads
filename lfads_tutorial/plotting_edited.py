...
def plot_lfads(x_txd, avg_lfads_dict, data_dict=None, dd_bidx=None,
  ...
  if data_dict is not None:
    #EDIT: Remove 'hiddens' because our data dict is not the same format and the renorm fun for now
    #ORIGINAL: true_rates = renorm_fun(data_dict['hiddens'][dd_bidx])
    true_rates = data_dict[dd_bidx]
    plt.subplot(366)
    plt.imshow(true_rates.T)
    plt.title('True rates')

  #EDIT: replace lograte_t with mean_t and logvar_t
  #ORIGINAL:
#   plt.subplot(365)
...
#   plt.title('rates')   
  ...
  plt.subplot(365)
  mean = remove_outliers(onp.exp(ld['mean_t']))
  plt.imshow(rates.T)
  plt.title('mean')   

  plt.subplot(365)
  variance = remove_outliers(onp.exp(ld['logvar_t']))
  plt.imshow(rates.T)
  plt.title('variance')    
  ...
  plt.plot(ii_mean, 'b')
  if data_dict is not None:
    #EDIT: remove 'inputs from data dict
    #ORIGINAL: true_input = data_dict['inputs'][dd_bidx]
    true_input = data_dict[dd_bidx]
