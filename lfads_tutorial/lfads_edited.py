...
def gru(params, h, x):
  ...
  #EDIT: add cell clipping
  #NOTE: could make this more programmatic by passing in the clip value as a hyperparameter, but that involves changing a lot of function signatures
  #ORIGINAL: return u * h + (1.0 - u) * c
  #######################################################  
  new_h = u * h + (1.0 - u) * c
  clip_value = 5.0
  new_h = np.clip(new_h, -clip_value, clip_value )
...
def lfads_params(key, lfads_hps):
  ...
  """
  #EDIT: change to 11 keys because need output for mean and standard deviation
  #ORIGINAL: key, skeys = utils.keygen(key, 10)
  ...
  #EDIT: changing lograte params to output mean and logvar, using linear transformation as per paper
  #ORIGINAL: lograte_params = affine_params(next(skeys), data_dim, factors_dim)
  #NOTE: output is data_dim * factors_dim because we need a rate for every neuron at every timestep
  #similarly need a mean and logvar for every channel at every timestep, so still data_dim x factors_dim
  mean = linear_params(next(skeys), data_dim, factors_dim)
  logvar = linear_params(next(skeys), data_dim, factors_dim)
  ...
  return {'ic_enc' : ic_enc_params,
          ...
          #EDIT: returning mean and logvar instead of logrates
          #ORIGINAL: 'logrates' : lograte_params
...
def lfads_decode_one_step(params, lfads_hps, key, keep_rate, c, f, g, xenc):
  ...
  #EDIT: output mean and logvar instead of lograte
  #ORIGINAL: lograte = affine(params['logrates'], f)
  #NOTE: doing a linear transformation according to the ECoG paper, but could also be affine as per LFADS paper
  mean = normed_linear(params['mean'], f)
  logvar = normed_linear(params['logvar'], f)
  #EDIT: return mean and logvar instead of lograte
  #ORIGINAL: return c, g, f, ii, ii_mean, ii_logvar, lograte
  return c, g, f, ii, ii_mean, ii_logvar, mean, logvar
...
def lfads_decode_one_step_scan(params, lfads_hps, keep_rate, state, key_n_xenc):
  ...
  state_and_returns = lfads_decode_one_step(params, lfads_hps, key, keep_rate,
                                            c, f, g, xenc)
  #EDIT: get mean and logvar instead of lograte
  #ORIGINAL: c, g, f, ii, ii_mean, ii_logvar, lograte = state_and_returns
  c, g, f, ii, ii_mean, ii_logvar, mean, logvar = state_and_returns
  state = (c, g, f)
...
def lfads(params, lfads_hps, key, x_t, keep_rate):
  ...
  #EDIT: get mean and logvar
  #ORIGINAL: c_t, gen_t, factor_t, ii_t, ii_mean_t, ii_logvar_t, lograte_t = \
  c_t, gen_t, factor_t, ii_t, ii_mean_t, ii_logvar_t, mean_t, logvar_t = \
      lfads_decode(params, lfads_hps, next(skeys), ic_mean, ic_logvar,
                   xenc_t, keep_rate)

  # As this is tutorial code, we're passing everything around.
  #EDIT: return mean and logvar
  return {'xenc_t' : xenc_t, 'ic_mean' : ic_mean, 'ic_logvar' : ic_logvar,
          ...
          #ORIGINAL'lograte_t' : lograte_t
          'mean_t' : mean_t, 'logvar_t' : logvar_t}


#EDIT: make params (arg 0) static in all of these
#ORIGINAL:
lfads_encode_jit = jit(lfads_encode)
lfads_decode_jit = jit(lfads_decode, static_argnums=(1,))
lfads_jit = jit(lfads, static_argnums=(1,))
#EDIT:
# lfads_encode_jit = jit(lfads_encode, static_argnums=(0,))
# lfads_decode_jit = jit(lfads_decode, static_argnums=(0,1,))
# lfads_jit = jit(lfads, static_argnums=(0,1,))
...
def lfads_losses(params, lfads_hps, key, x_bxt, kl_scale, keep_rate):
  ...
  #EDIT: change get mean and logvar instead of lograte
  #ORIGINAL: lograte_bxt = lfads['lograte_t']
  mean_bxt = lfads['mean_t']
  logvar_bxt = lfads['logvar_t']
  #EDIT: sample from gaussian, also scale to 1/100
  #ORIGINAL: log_p_xgz = np.sum(dists.poisson_log_likelihood(x_bxt, lograte_bxt)) / B
  #NOTE: what does p_xgz stand for
  ll_gaussian = np.sum(dists.diag_gaussian_log_likelihood(x_bxt, mean_bxt, logvar_bxt, lfads_hps['var_min'])) / B
  ...
  #EDIT: put gaussian log likelihood in sum instead
  #ORIGINAL: loss = -log_p_xgz + kl_loss_g0 + kl_loss_ii + l2_loss
  loss = -ll_gaussian + kl_loss_g0 + kl_loss_ii + l2_loss

  #EDIT: return gaussian log likelihood
  all_losses = {'total' : loss,
                #ORIGINAL: 'nlog_p_xgz' : -log_p_xgz,
...
#EDIT:
# batch_lfads_jit = jit(batch_lfads, static_argnums=(0,1,))
# lfads_losses_jit = jit(lfads_losses, static_argnums=(0,1,))
# lfads_training_loss_jit = jit(lfads_training_loss, static_argnums=(0,1,))
# posterior_sample_and_average_jit = jit(posterior_sample_and_average, static_argnums=(0,1,))
